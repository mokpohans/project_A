...
class linearmenu: # 현재 사용중인 메뉴판; 예정 -> 각 버튼을 누르면 toplevel이 뜨고 해당 액티비티클래스가 뜨도록 할 함수 작성 예정
    ...
    def __init__(self, parent, partitions, index: any = None, texts=[], command=None, width=800, height=70):
        self._parent = parent
        ...
        self._buttoncreate(self._linearbase, self._partitions, index=index, texts=self._texts, command=self._command)


    def _buttoncreate(self, parent, amount, index:any=None, texts=[], command=None):
        for i in range(0, amount):
            ...
            if not texts: #텍스트s에 값이 없을 때
                ...
                #globals에서 버튼 번호별로 생성 -> 번호가 겹치면서 list에 쌓인다. 속성이 겹치는지 확인해 볼 것
                    globals()[f'__{index}button{i}'] = ttk.Button(parent, text=texts[i], command=command)
                    globals()[f'__{index}button{i}'].grid(row=0, column=i, sticky=tk.NSEW)
                elif():
                    print("error occured, please put 'str' type in texts by a list")

    def elementManipulate(self, index: any = None, seq: int = 0, command=None, compound=None, cursor=None,
                          image=None, style=None, takefocus=None, text=None, textvariable=None,
                          underline=None, width=None):
        if(seq > self._partitions):
            print("Error: Out of Range! Check your index number & partitions number")
        elif(seq <= self._partitions):
            globals()[f'__{index}button{seq}'].configure(command=command, compound=compound, cursor=cursor,
                                                  image=image, style=style, takefocus=takefocus,
                                                  text=text, textvariable=textvariable,
                                                  underline=underline, width=width)
            globals()[f'__{index}button{seq}'].update()
            print(f"in {index}-{seq} : {globals()[f'__{index}button{seq}']}, mainpulated button text is {text}")
